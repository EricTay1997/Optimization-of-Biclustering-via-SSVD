...
@jit(nopython=True)
def ssvd(X, BIC_v = BIC_v, BIC_u = BIC_u, tol = 1e-3, lambda_us = None, 
         lambda_vs = None, gamma1s=None, gamma2s=None, max_iter = 20):
        ...
        if not v_hat.any():
            ...
            for gamma2 in gamma2s:
                ...
                for i in range(lambda_vs.shape[0]):
                    bic_val, v_val = BIC_v(lambda_v=lambda_vs[i,0], v_hat=v_hat, w2=w2, u=u, 
                                           sigma_sq_hat=sigma_sq_hat, nd=nd, X=X)
        ...
        if not u_hat.any():
            ...
            for gamma1 in gamma1s:
                ...
                for i in range(lambda_us.shape[0]):
                    bic_val, u_val = BIC_u(lambda_u = lambda_us[i,0], u_hat=u_hat, w1=w1, v=v, 
                                           sigma_sq_hat=sigma_sq_hat, nd=nd, X=X)
                    BIC_vals_lst[i] = bic_val
                    u_vals[i] = u_val
